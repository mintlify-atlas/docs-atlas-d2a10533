...
Every node in the router utils (not just leaf procedures) also exposes general query utilities:

### `.invalidateQueries(queryClient, options?)`

Invalidates all queries under this router node.
...
await utils.planet.invalidateQueries(queryClient)

// Invalidate only list queries:
await utils.planet.list.invalidateQueries(queryClient)
```

### `.setQueryData(queryClient, input, updater)`

Manually sets query data in the cache.

```typescript
utils.planet.find.setQueryData(queryClient, { id: 1 }, { id: 1, name: 'Earth' })
```

### `.getQueryData(queryClient, input)`

Reads query data from the cache.

```typescript
const planet = utils.planet.find.getQueryData(queryClient, { id: 1 })
```
