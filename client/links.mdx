...
```typescript
import { RPCLink } from '@orpc/client/fetch'
import { ClientRetryPlugin } from '@orpc/client'
...
|--------|--------|-------------|
| `BatchPlugin` | `@orpc/client` | Batches multiple procedure calls into a single HTTP request. |
| `DedupeRequestsPlugin` | `@orpc/client` | Deduplicates identical in-flight requests. |
| `RetryAfterPlugin` | `@orpc/client` | Respects the `Retry-After` header from 429/503 responses. |
| `SimpleCsrfProtectionPlugin` | `@orpc/client` | Adds a custom header to guard against CSRF attacks. |
